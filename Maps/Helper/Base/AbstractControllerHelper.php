...
use TdM\MapsModule\Helper\ImageHelper;
use TdM\MapsModule\Helper\ModelHelper;
...
abstract class AbstractControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in TdMMapsModule.
     *
    ...
    public function getObjectTypes($context = '', array $args = [])
    {
        ...
        $allowedObjectTypes = [];
        $allowedObjectTypes[] = 'map';
    
        return $allowedObjectTypes;
    }

    /**
     * Returns the default object type in TdMMapsModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        $templateParameters['own'] = $request->query->getInt('own', $this->variableApi->get('TdMMapsModule', 'showOnlyOwnEntries', 0));
        ...
        if ($templateParameters['all'] != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('TdMMapsModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('TdM\MapsModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('tdmmapsmodule_' . strtolower($objectType) . '_view', $urlParameters);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('tdmmapsmodule_' . strtolower($objectType) . '_display', $urlParameters);
        }
    ...
    public function addTemplateParameters($objectType = '', array $parameters = [], $context = '', array $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if ($objectType == 'map') {
                $thumbRuntimeOptions = [];
                $thumbRuntimeOptions[$objectType . 'MapFile'] = $this->imageHelper->getRuntimeOptions($objectType, 'mapFile', $context, $args);
                $thumbRuntimeOptions[$objectType . 'Foreground'] = $this->imageHelper->getRuntimeOptions($objectType, 'foreground', $context, $args);
                $thumbRuntimeOptions[$objectType . 'Underground'] = $this->imageHelper->getRuntimeOptions($objectType, 'underground', $context, $args);
            ...
            if (in_array($args['action'], ['display', 'edit', 'view'])) {
                ...
                $parameters['relationThumbRuntimeOptions'] = $this->imageHelper->getCustomRuntimeOptions('', '', 'TdMMapsModule_relateditem', $context, $args);
            }
        }
