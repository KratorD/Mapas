...
namespace TdM\MapsModule\Form\Handler\Map\Base;

use TdM\MapsModule\Form\Handler\Common\EditHandler;
use TdM\MapsModule\Form\Type\MapType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'map';
        $this->objectTypeCapital = 'Map';
        $this->objectTypeLower = 'map';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the map yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'TdMMapsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(MapType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of map area
        $codes[] = 'userIndex';
        // admin index page of map area
        $codes[] = 'adminIndex';
        // user list of maps
        $codes[] = 'userView';
        // admin list of maps
        $codes[] = 'adminView';
        // user list of own maps
        $codes[] = 'userOwnView';
        // admin list of own maps
        $codes[] = 'adminOwnView';
        // user detail page of treated map
        $codes[] = 'userDisplay';
        // admin detail page of treated map
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        $routePrefix = 'tdmmapsmodule_' . $this->objectTypeLower . '_' . $routeArea;
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Map created.');
                } else {
                    $message = $this->__('Done! Map updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Map deleted.');
                break;
            default:
                $message = $this->__('Done! Map updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'TdMMapsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'map', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        if ($this->request->getSession()->has('tdmmapsmodule' . $this->objectTypeCapital . 'Referer')) {
            $this->request->getSession()->remove('tdmmapsmodule' . $this->objectTypeCapital . 'Referer');
        }
