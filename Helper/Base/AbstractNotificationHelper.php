...
abstract class AbstractNotificationHelper
{
    ...
    protected function collectRecipients($debug = false)
    {
        ...
        if ($this->recipientType == 'moderator' || $this->recipientType == 'superModerator') {
            ...
            $moderatorGroupId = $this->variableApi->get('TdMMapsModule', 'moderationGroupFor' . $modVarSuffix, GroupsConstant::GROUP_ID_ADMIN);
            if ($this->recipientType == 'superModerator') {
                $moderatorGroupId = $this->variableApi->get('TdMMapsModule', 'superModerationGroupFor' . $modVarSuffix, GroupsConstant::GROUP_ID_ADMIN);
            }
    ...
    protected function sendMails()
    {
        ...
        foreach ($this->recipients as $recipient) {
            ...
            $body = $this->templating->render('@TdMMapsModule/' . $template, [
                'recipient' => $recipient,
                'mailData' => $mailData
            ]);
            $altBody = '';
    ...
    protected function prepareEmailData()
    {
        ...
        $hasDisplayAction = in_array($objectType, ['map']);
        $hasEditAction = in_array($objectType, ['map']);
        $routeArea = in_array($this->recipientType, ['moderator', 'superModerator']) ? 'admin' : '';
        $routePrefix = 'tdmmapsmodule_' . strtolower($objectType) . '_' . $routeArea;
