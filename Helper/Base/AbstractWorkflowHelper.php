...
abstract class AbstractWorkflowHelper
{
    ...
    public function executeAction(EntityAccess $entity, $actionId = '', $recursive = false)
    {
        ...
        $logArgs = ['app' => 'TdMMapsModule', 'user' => $this->currentUserApi->get('uname')];
    ...
    public function collectAmountOfModerationItems()
    {
        ...
        $state = 'waiting';
        $objectType = 'map';
        if ($this->permissionApi->hasPermission('TdMMapsModule:' . ucfirst($objectType) . ':', '::', ACCESS_ADD)) {
            $amount = $this->getAmountOfModerationItems($objectType, $state);
            if ($amount > 0) {
                $amounts[] = [
                    'aggregateType' => 'mapsApproval',
                    'description' => $this->translator->__('Maps pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->translator->_fn('One map is waiting for approval.', '%amount% maps are waiting for approval.', $amount, ['%amount%' => $amount])
                ];
        
                $this->logger->info('{app}: There are {amount} {entities} waiting for approval.', ['app' => 'TdMMapsModule', 'amount' => $amount, 'entities' => 'maps']);
            }
        }
