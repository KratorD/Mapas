...
abstract class AbstractMapController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('tdmmapsmodule_map_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $viewHelper = $this->get('tdm_maps_module.view_helper');
        
        $request->query->set('sort', $sort);
        $request->query->set('sortdir', $sortdir);
        $request->query->set('pos', $pos);
        
        $sortableColumns = new SortableColumns($this->get('router'), 'tdmmapsmodule_map_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
        ...
        $sortableColumns->addColumns([
            ...
            new Column('author'),
            new Column('mapFile'),
            new Column('testState'),
            new Column('game'),
            new Column('sizeMap'),
            new Column('bUnderground'),
            new Column('languageMap'),
            new Column('createDat'),
            new Column('versionMap'),
            new Column('difficulty'),
            new Column('nHumans'),
            new Column('nPlayers'),
            new Column('gameType'),
            new Column('mapStyle'),
            new Column('description'),
            new Column('foreground'),
            new Column('underground'),
            new Column('scoreRev'),
            new Column('nScoreRev'),
            new Column('nDownloads'),
        ...
        $filteredEntities = [];
        foreach ($templateParameters['items'] as $map) {
            if (!$this->hasPermission('TdMMapsModule:' . ucfirst($objectType) . ':', $map->getKey() . '::', $permLevel)) {
                continue;
            }
    ...
    {
        return $this->displayInternal($request, $map, true);
    }
    ...
    {
        return $this->displayInternal($request, $map, false);
    }
    
    /**
     * This method includes the common implementation code for adminDisplay() and display().
     */
    protected function displayInternal(Request $request, MapEntity $map, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'map';
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('tdm_maps_module.form.handler.map');
        ...
        // fetch and return the appropriate template
        return $this->get('tdm_maps_module.view_helper')->processTemplate($objectType, 'edit', $templateParameters);
    }
    /**
    ...
    {
        return $this->deleteInternal($request, $map, true);
    }
    ...
    {
        return $this->deleteInternal($request, $map, false);
    }
    
    /**
     * This method includes the common implementation code for adminDelete() and delete().
     */
    protected function deleteInternal(Request $request, MapEntity $map, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'map';
        ...
        $logArgs = ['app' => 'TdMMapsModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'map', 'id' => $map->getKey()];
        
        // determine available workflow actions
        $workflowHelper = $this->get('tdm_maps_module.workflow_helper');
        $actions = $workflowHelper->getActionsForObject($map);
        ...
        // redirect to the list of maps
        $redirectRoute = 'tdmmapsmodule_map_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        if (!$deleteAllowed) {
            $this->addFlash('error', $this->__('Error! It is not allowed to delete this map.'));
            $logger->error('{app}: User {user} tried to delete the {entity} with id {id}, but this action was not allowed.', $logArgs);
        
            return $this->redirectToRoute($redirectRoute);
        }
        
        $form = $this->createForm('Zikula\Bundle\FormExtensionBundle\Form\Type\DeletionType', $map);
        $hookHelper = $this->get('tdm_maps_module.hook_helper');
        
        // Call form aware display hooks
        $formHook = $hookHelper->callFormDisplayHooks($form, $map, FormAwareCategory::TYPE_DELETE);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                // Let any ui hooks perform additional validation actions
                $validationErrors = $hookHelper->callValidationHooks($map, UiHooksCategory::TYPE_VALIDATE_DELETE);
                if (count($validationErrors) > 0) {
                    ...
                    $success = $workflowHelper->executeAction($map, $deleteActionId);
                    if ($success) {
                    ...
                    $hookHelper->callFormProcessHooks($form, $map, FormAwareCategory::TYPE_PROCESS_DELETE);
                    
                    // Let any ui hooks know that we have deleted the map
                    $hookHelper->callProcessHooks($map, UiHooksCategory::TYPE_PROCESS_DELETE);
        ...
        // fetch and return the appropriate template
        return $this->get('tdm_maps_module.view_helper')->processTemplate($objectType, 'delete', $templateParameters);
    }
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'TdMMapsModule', 'user' => $userName, 'action' => $action, 'entity' => 'map', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'TdMMapsModule', 'user' => $userName, 'entity' => 'map', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'TdMMapsModule', 'user' => $userName, 'action' => $action, 'entity' => 'map', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('tdmmapsmodule_map_display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('tdmmapsmodule_map_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
